...
APP_CACHE_VERSION = "verstossliste-jahresfilter-2026-05-05-v14-warme-farben"
...
.tour-row:hover td{ background:#fff7ed; }"""
...
function spesenPdfOne(name) {
  ...
  var css = "@page{size:A4 portrait;margin:11mm}"
    ...
    + "thead tr{background:#431407;color:#fff}"
...
function spesenShowLateStarts() {
  ...
  if(!monthKeys.length){
    html += "<div style='margin-top:12px;padding:32px;text-align:center;color:#94a3b8;font-size:13px;background:#fffaf0;border:1px dashed #cbd5e1;border-radius:8px;'>Keine Spätstarts im gewählten Zeitraum.</div>";
    html += "</div>";
    panel.innerHTML = html;
    panel.scrollTop = 0;
    return;
  ...
  cards.forEach(function(c){
    html += "<div style='background:#fffaf0;border:1px solid #e2e8f0;border-radius:8px;padding:10px 12px;'>"
  ...
  monthKeys.forEach(function(k){
    ...
    html += "<div style='padding:10px 14px;border-bottom:1px solid #f1f5f9;background:#fffaf0;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;'>";
...
function fwRenderRanking() {
  ...
  html += "<thead><tr style='position:sticky;top:0;background:linear-gradient(180deg,#9a3412 0%,#7c2d12 100%);z-index:2;box-shadow:0 2px 4px rgba(0,0,0,.08);'>";
...
function fwRenderTotalBanner() {
  ...
  wrap.innerHTML =
    "<div style=\"background:linear-gradient(135deg,#9a3412 0%,#c2410c 45%,#a16207 72%,#16a34a 100%);border-radius:14px;padding:22px 28px;color:#fff;box-shadow:0 6px 20px rgba(15,23,42,.15);display:flex;align-items:center;justify-content:space-between;gap:20px;flex-wrap:wrap;\">" +
...
(function(){
  ...
  function _fwBuildHeaderHTML() {
    var html = "<style>.fw-row{transition:background .1s}.fw-row:hover{background:#fff7ed!important}</style>";
  ...
  function _fwUpdateMoreButton() {
    ...
    more.innerHTML = "<button onclick='window._fwLoadMore()' style='padding:6px 14px;border:1px solid #9a3412;background:#fff7ed;color:#9a3412;border-radius:4px;cursor:pointer;font-weight:700;font-size:11px;'>" + label + "</button>";
  }
...
function verstossRenderGraph() {
  ...
  html += verstossGraphKpi("Stärkster Monat", maxMonthName + (maxMonth ? " · " + maxMonth : ""), "#431407", "&#128197;");
  html += "</div>";
  ...
  html += "<table style='width:100%;border-collapse:collapse;font-size:12px;'><thead><tr style='background:#431407;color:#fff;'>";
  ...
  months.forEach(function(m, i) {
    ...
    html += "<td style='padding:8px 10px;text-align:right;font-weight:900;color:#431407;'>" + monthCount[i] + "</td>";
  ...
  verstossChart("verstoss-chart-type", {
    ...
    data: {
      ...
      datasets: [{ data: topTypes.map(function(x){ return x[1]; }), backgroundColor: ["#dc2626", "#f97316", "#f59e0b", "#431407", "#9a3412", "#b45309", "#16a34a", "#cbd5e1", "#be123c", "#475569", "#854d0e", "#92400e"] }]
    },
  ...
  verstossChart("verstoss-chart-driver", {
    type: "bar",
    data: { labels: topDrivers.map(function(x){ return x[0]; }), datasets: [{ label: "Verstöße", data: topDrivers.map(function(x){ return x[1]; }), backgroundColor: "#431407", borderRadius: 5 }] },
...
function verstossRender() {
  ...
  if (stats) {
    ...
    stats.innerHTML =
      ...
      + (sinceLabel ? " <span style='display:inline-flex;align-items:center;gap:6px;background:#fffaf0;border:1px solid #e2e8f0;border-radius:999px;padding:5px 10px;color:#475569;'>seit <b>" + verstossEsc(sinceLabel) + "</b></span>" : "");
  }
  ...
  function verstossMiniCard(label, value, sub, accent, bg, icon) {
    return "<div style='background:" + bg + ";border:1px solid " + accent + "33;border-left:5px solid " + accent + ";border-radius:14px;padding:12px 14px;box-shadow:0 8px 20px rgba(15,23,42,.05);min-width:0;'>"
      + "<div style='display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:5px;'>"
      + "<div style='font-size:10.5px;font-weight:900;color:#64748b;text-transform:uppercase;letter-spacing:.45px;'>" + verstossEsc(label) + "</div>"
      + "<div style='font-size:18px;line-height:1;'>" + icon + "</div>"
      + "</div>"
      + "<div style='font-size:22px;font-weight:950;color:#0f172a;letter-spacing:-.6px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;'>" + value + "</div>"
      + "<div style='font-size:11px;font-weight:750;color:#64748b;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;'>" + verstossEsc(sub || "") + "</div>"
      + "</div>";
  }
  ...
  function sortableTh(key, label, align) {
    ...
    return "<th onclick='verstossSort(\"" + key + "\")' "
         + "style='padding:9px 10px;text-align:" + (align || "left") + ";color:" + (active ? "#991b1b" : "#475569") + ";"
         + "font-size:10px;font-weight:950;letter-spacing:.45px;text-transform:uppercase;cursor:pointer;user-select:none;white-space:nowrap;border-bottom:1px solid #e2e8f0;background:" + (active ? "#fee2e2" : "#f8fafc") + ";' "
         + "title='Klicken zum Sortieren'>" + verstossEsc(label) + arrow + "</th>";
  }

  function vsPenaltyPill(amount, color) {
    amount = Number(amount) || 0;
    if (!amount) return "<span style='color:#cbd5e1;font-weight:850;'>—</span>";
    return "<span style='display:inline-flex;justify-content:flex-end;min-width:76px;background:#fff;border:1px solid " + color + "33;color:" + color + ";border-radius:999px;padding:3px 9px;font-weight:950;font-variant-numeric:tabular-nums;'>" + verstossFmtEuro(amount) + "</span>";
  }
  ...
  var html = "";
  html += "<div style='display:grid;grid-template-columns:repeat(4,minmax(160px,1fr));gap:10px;margin:0 0 12px 0;'>";
  html += verstossMiniCard("Verstöße", String(totV), drivers.length + " Fahrer in der Liste", "#dc2626", "#fff7f7", "&#9888;&#65039;");
  html += verstossMiniCard("Bußgeld Fahrer", verstossFmtEuro(totDP), "Summe Fahrer", "#ef4444", "#fff7ed", "&#128179;");
  html += verstossMiniCard("Bußgeld Firma", verstossFmtEuro(totCP), "Summe Firma", "#b45309", "#fffbeb", "&#127970;");
  html += verstossMiniCard("Aktuellster Tag", latestDisplay, topDriver ? ("Top: " + topDriver.name + " · " + topDriver.count) : "Keine Daten", "#431407", "#f8fafc", "&#128197;");
  html += "</div>";
  ...
  html += "<th style='padding:9px 6px;width:34px;background:#fffaf0;border-bottom:1px solid #e2e8f0;'></th>";
  ...
  drivers.forEach(function(d, i) {
    ...
    var rowBg = isOpen ? "#f8fafc" : (isRecent ? "#fff7f7" : (i % 2 === 0 ? "#ffffff" : "#f8fafc"));
    var countBg = d.count >= 10 ? "#fee2e2" : "#ffedd5";
    var countColor = d.count >= 10 ? "#991b1b" : "#7c2d12";

    html += "<tr onclick='verstossToggleDriver(" + JSON.stringify(d.name) + ")' "
          + "style='background:" + rowBg + ";cursor:pointer;transition:background .12s,box-shadow .12s;border-left:5px solid " + (isRecent ? "#dc2626" : (isOpen ? "#431407" : "transparent")) + ";' "
          + "onmouseover=\"this.style.background='#eef6ff'\" onmouseout=\"this.style.background='" + rowBg + "'\">";
    html += "<td style='padding:11px 4px 11px 10px;text-align:center;color:" + (isOpen ? "#431407" : "#94a3b8") + ";font-size:12px;border-bottom:1px solid #eef2f7;'>" + (isOpen ? "&#9660;" : "&#9654;") + "</td>";
    html += "<td style='padding:11px 10px;border-bottom:1px solid #eef2f7;'>"
          + "<div style='display:flex;align-items:center;gap:8px;min-width:0;'>"
          + "<span style='width:28px;height:28px;border-radius:999px;background:#f1f5f9;border:1px solid #e2e8f0;display:inline-flex;align-items:center;justify-content:center;font-size:12px;font-weight:950;color:#475569;flex:0 0 auto;'>" + verstossEsc((d.name || "?").substring(0,1).toUpperCase()) + "</span>"
          + "<div style='min-width:0;'><div style='font-weight:950;color:#0f172a;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;'>" + verstossEsc(d.name) + "</div>"
          + "<div style='font-size:10.5px;font-weight:750;color:#64748b;margin-top:1px;'>Klicken zum Öffnen</div></div>"
          + "</div></td>";
    html += "<td style='padding:11px 10px;text-align:right;border-bottom:1px solid #eef2f7;'>"
          + "<span style='display:inline-flex;align-items:center;justify-content:center;min-width:42px;background:" + countBg + ";color:" + countColor + ";border-radius:999px;padding:4px 10px;font-size:12px;font-weight:950;font-variant-numeric:tabular-nums;'>" + d.count + "</span></td>";
    html += "<td style='padding:11px 10px;border-bottom:1px solid #eef2f7;color:" + (isRecent ? "#991b1b" : "#334155") + ";font-weight:850;white-space:nowrap;font-variant-numeric:tabular-nums;'>"
          + verstossEsc(lastStart || "—")
          + (isRecent ? " <span style='display:inline-flex;align-items:center;background:#dc2626;color:#fff;border-radius:999px;padding:2px 7px;font-size:9px;font-weight:950;margin-left:6px;letter-spacing:.3px;'>NEU</span>" : "")
          + "</td>";
    html += "<td style='padding:11px 10px;text-align:right;border-bottom:1px solid #eef2f7;white-space:nowrap;'>" + vsPenaltyPill(d.sum_driver_penalty, "#dc2626") + "</td>";
    html += "<td style='padding:11px 12px 11px 10px;text-align:right;border-bottom:1px solid #eef2f7;white-space:nowrap;'>" + vsPenaltyPill(d.sum_company_penalty, "#b45309") + "</td>";
    html += "</tr>";

    if (isOpen) {
      html += "<tr style='background:#f1f5f9;'>";
      html += "<td colspan='6' style='padding:0 12px 14px 12px;border-bottom:1px solid #cbd5e1;background:#f1f5f9;'>";
      html += "<div style='background:#fffaf0;border:1px solid #cbd5e1;border-radius:14px;margin-top:10px;overflow:hidden;box-shadow:0 8px 22px rgba(15,23,42,.10);'>";
      ...
      html += "<div style='display:flex;align-items:center;gap:10px;padding:12px 16px;border-bottom:1px solid #cbd5e1;background:linear-gradient(180deg,#f8fafc 0%,#f8fafc 100%);flex-wrap:wrap;'>";
      ...
      if (d.types && d.types.length) {
        html += "<div style='display:flex;align-items:center;gap:7px;padding:10px 16px;border-bottom:1px solid #e2e8f0;background:#ffffff;flex-wrap:wrap;'>";
        ...
        html += "<button onclick='event.stopPropagation();verstossResetTypeFilter()' "
              + "style='background:" + (!_vsTypeFilter ? "#431407" : "#fff") + ";border:1px solid " + (!_vsTypeFilter ? "#431407" : "#cbd5e1") + ";color:" + (!_vsTypeFilter ? "#fff" : "#475569") + ";border-radius:999px;padding:4px 10px;font-size:10.5px;font-weight:950;cursor:pointer;font-family:inherit;'>Alle</button>";
        d.types.forEach(function(t) {
          ...
          html += "<button onclick='event.stopPropagation();verstossFilterType(" + JSON.stringify(String(t[0])) + ")' "
                + "style='background:" + (activeType ? "#fee2e2" : "#fff") + ";border:1px solid " + (activeType ? "#ef4444" : "#cbd5e1") + ";color:" + (activeType ? "#991b1b" : "#475569") + ";border-radius:999px;padding:4px 10px;font-size:10.5px;font-weight:850;cursor:pointer;font-family:inherit;'>"
                + verstossEsc(t[0]) + " <b style='margin-left:4px;'>" + t[1] + "</b></button>";
        });
        html += "</div>";
      }

      html += "<div style='max-height:460px;overflow:auto;background:#ffffff;'>";
      html += "<table style='width:100%;border-collapse:collapse;font-size:11.5px;'>";
      html += "<thead><tr style='position:sticky;top:0;z-index:2;background:#e2e8f0;color:#334155;box-shadow:0 2px 6px rgba(15,23,42,.08);'>";
      ["Start", "Ende", "Verstoß", "Soll", "Ist", "Differenz", "Bußgeld Fahrer", "Bußgeld Firma"].forEach(function(h, idx) {
        ...
        html += "<th style='padding:8px 10px;text-align:" + align + ";font-size:10px;font-weight:950;letter-spacing:.35px;white-space:nowrap;border-right:1px solid #cbd5e1;border-bottom:1px solid #cbd5e1;'>" + h + "</th>";
      });
      html += "</tr></thead><tbody>";

      if (!detailList.length) {
        html += "<tr><td colspan='8' style='padding:22px;text-align:center;color:#64748b;font-weight:800;background:#ffffff;'>Keine Verstöße für diesen Filter.</td></tr>";
      }
      ...
      detailList.forEach(function(v, j) {
        ...
        if (monthLabel !== currentMonth) {
          ...
          html += "<tr style='background:#f1f5f9;'><td colspan='8' style='padding:8px 12px;color:#431407;font-size:10px;font-weight:950;text-transform:uppercase;letter-spacing:.55px;border-top:1px solid #fed7aa;border-bottom:1px solid #fed7aa;'>" + verstossEsc(monthLabel) + "</td></tr>";
        }
        var jbg = j % 2 === 0 ? "#ffffff" : "#f8fafc";
        var diffPositive = (Number(v.diff) || 0) > 0;
        html += "<tr style='background:" + jbg + ";border-bottom:1px solid #e2e8f0;'>";
        html += "<td style='padding:8px 10px;color:#0f172a;font-weight:850;white-space:nowrap;font-variant-numeric:tabular-nums;'>" + verstossEsc(v.start || "—") + "</td>";
        html += "<td style='padding:8px 10px;color:#475569;font-weight:750;white-space:nowrap;font-variant-numeric:tabular-nums;'>" + verstossEsc(v.end || "—") + "</td>";
        html += "<td style='padding:8px 10px;color:#334155;min-width:260px;'>"
              ...
              + "</td>";
        html += "<td style='padding:8px 10px;text-align:right;color:#475569;font-weight:800;white-space:nowrap;font-variant-numeric:tabular-nums;'>" + verstossFmtMin(v.target) + "</td>";
        html += "<td style='padding:8px 10px;text-align:right;color:#475569;font-weight:800;white-space:nowrap;font-variant-numeric:tabular-nums;'>" + verstossFmtMin(v.ist) + "</td>";
...
function verstossPdfOne(name) {
  ...
  var css = "@page{size:A4 portrait;margin:12mm}"
    ...
    + ".types{margin:4mm 0;padding:3mm 4mm;background:#fffaf0;border:1px solid #e2e8f0;border-radius:2mm}"
  ...
  body += "<div class='kpi'>"
    + "<div><div class='num' style='color:#431407;'>" + d.count + "</div><div class='lbl'>Verstöße gesamt</div></div>"
...
.nav-btn.active{{background:linear-gradient(135deg,#9a3412 0%,#c2410c 55%,#a16207 100%);border-color:#c2410c;color:#fff;box-shadow:0 3px 10px rgba(194,65,12,.24)}}
...
.nav-dd-btn.active{{background:linear-gradient(135deg,#9a3412 0%,#c2410c 55%,#a16207 100%);border-color:#c2410c;color:#fff;box-shadow:0 3px 10px rgba(194,65,12,.24)}}
...
.dd-item.active{{background:linear-gradient(135deg,#9a3412 0%,#c2410c 55%,#a16207 100%);color:#fff}}
.topnav-stamp{{
...
.vz-day-btn{{padding:7px 14px;border:1.5px solid #fdba74;background:#fff;color:#9a3412;border-radius:7px;cursor:pointer;font-weight:800;font-size:12px;font-family:'Segoe UI',Arial,sans-serif;transition:all .15s;letter-spacing:.1px}}
.vz-day-btn:hover{{background:#fff7ed;border-color:#9a3412}}
.vz-day-btn.active{{background:linear-gradient(135deg,#9a3412 0%,#c2410c 55%,#a16207 100%);color:#fff;border-color:#7c2d12;box-shadow:0 2px 7px rgba(194,65,12,.28)}}
</style>
</head>
<body>
...
<div class="frame-wrap">
  ...
  <div id="panel-vz" style="display:none;flex:1;overflow-y:auto;padding:18px 18px 28px;background:linear-gradient(135deg,#fff7ed 0%,#fff7ed 52%,#f0fdf4 100%);font-family:'Segoe UI',Arial,sans-serif">
    <div style="width:100%;max-width:1728px;margin:0 auto">

      <!-- ── Header-Karte mit Titel + Aktionen ─────────────────────────────── -->
      <div style="background:linear-gradient(180deg,#ffffff 0%,#f0f9ff 100%);border:1px solid #fdba74;border-radius:12px;padding:18px 22px;box-shadow:0 3px 14px rgba(14,165,233,.16);margin-bottom:18px">
        <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;margin-bottom:16px">
          <div style="display:flex;align-items:center;gap:12px;min-width:0;flex:1">
            <div style="width:40px;height:40px;border-radius:10px;background:linear-gradient(135deg,#9a3412 0%,#c2410c 58%,#a16207 100%);display:flex;align-items:center;justify-content:center;font-size:20px;box-shadow:0 2px 7px rgba(194,65,12,.26);flex-shrink:0">&#128703;</div>
        ...
        <div style="display:flex;gap:18px;align-items:flex-start;flex-wrap:wrap;padding-top:14px;border-top:1px solid #eef2f7">
          <div style="flex:1;min-width:280px">
            <div style="font-size:11px;font-weight:800;color:#9a3412;margin-bottom:8px;letter-spacing:.4px;text-transform:uppercase">Tag</div>
          ...
          <div style="flex-shrink:0">
            <div style="font-size:11px;font-weight:800;color:#9a3412;margin-bottom:8px;letter-spacing:.4px;text-transform:uppercase">Datum</div>
            <input id="fw-date-picker" type="date" onchange="fwSetDate(this.value)"
              style="padding:9px 12px;border:1.5px solid #fdba74;border-radius:7px;font-size:13px;font-weight:700;font-family:inherit;outline:none;background:#fff;color:#0b1220;min-width:180px;box-shadow:inset 0 1px 2px rgba(15,23,42,.04)">
          </div>
        </div>
      </div>
      ...
      <!-- ── Übersicht-Karte ───────────────────────────────────────────────── -->
      <div style="background:#fff;border:1px solid #fdba74;border-radius:12px;box-shadow:0 3px 14px rgba(14,165,233,.14);overflow:hidden;margin-bottom:14px">

        <div style="display:flex;justify-content:space-between;align-items:center;gap:14px;flex-wrap:wrap;padding:16px 20px;border-bottom:1px solid #eef2f7;background:linear-gradient(180deg,#fffaf0 0%,#ffffff 100%)">
          <div style="display:flex;align-items:center;gap:10px">
            <div style="width:32px;height:32px;border-radius:8px;background:#ffedd5;color:#9a3412;display:flex;align-items:center;justify-content:center;font-size:15px">&#128203;</div>
            <div>
        ...
        <div style="padding:14px 20px;border-bottom:1px solid #eef2f7;background:#fbfcfd">
          <select id="fw-overview-driver" onchange="fwRenderOverview()"
            ...
            onfocus="this.style.borderColor='#9a3412'" onblur="this.style.borderColor='#fdba74'"></select>
        </div>
      ...
      <!-- ── Rangliste ───────────────────────────────────────────────────── -->
      <div style="background:#fff;border:1px solid #fed7aa;border-radius:12px;box-shadow:0 2px 10px rgba(14,165,233,.12);overflow:hidden">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:14px;flex-wrap:wrap;padding:16px 20px;border-bottom:1px solid #eef2f7;background:linear-gradient(180deg,#fffaf0 0%,#ffffff 100%)">
          <div style="display:flex;align-items:center;gap:10px">
      ...
      <!-- ── LKW-Rangliste ───────────────────────────────────────────────── -->
      <div style="background:#fff;border:1px solid #fed7aa;border-radius:12px;box-shadow:0 2px 10px rgba(14,165,233,.12);overflow:hidden;margin-top:14px">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:14px;flex-wrap:wrap;padding:16px 20px;border-bottom:1px solid #eef2f7;background:linear-gradient(180deg,#fffaf0 0%,#ffffff 100%)">
          <div style="display:flex;align-items:center;gap:10px">
            <div style="width:32px;height:32px;border-radius:8px;background:#ffedd5;color:#9a3412;display:flex;align-items:center;justify-content:center;font-size:17px">&#128666;</div>
            <div>
  ...
  <div id="panel-tel" style="display:none;flex:1;overflow-y:auto;padding:30px;background:#e8ecf1;font-family:'Segoe UI',Arial,sans-serif">
    <div style="max-width:900px;margin:0 auto">
      <h2 style="color:#9a3412;font-size:18px;font-weight:900;margin:0 0 4px 0">&#128222; Telefonliste</h2>
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;flex-wrap:wrap">
        <input id="tel-search" placeholder="Name suchen..." oninput="telFilter(this.value)"
          style="flex:1;min-width:200px;max-width:400px;padding:8px 14px;border:2px solid #9a3412;
  ...
  <div id="panel-sam" style="display:none;flex:1;overflow-y:auto;padding:30px;background:#e8ecf1;font-family:Segoe UI,Arial,sans-serif">
    <div style="max-width:1000px;margin:0 auto">
      <h2 style="color:#9a3412;font-size:18px;font-weight:900;margin:0 0 4px 0">&#128664; Sa + So Einsätze</h2>
      ...
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;flex-wrap:wrap">
        <input id="sam-search" placeholder="Fahrer suchen..." oninput="samFilter(this.value)"
          style="flex:1;min-width:180px;max-width:280px;padding:7px 14px;border:2px solid #9a3412;
                 border-radius:5px;font-size:13px;font-family:inherit;outline:none">
        <select id="sam-year-sel" onchange="samYearChange(this.value)"
          style="padding:7px 12px;border:2px solid #9a3412;border-radius:5px;font-size:12px;font-weight:700;
                 color:#9a3412;cursor:pointer;font-family:inherit;outline:none;background:#fff;">
          </select>
        <div style="display:flex;gap:5px;">
          <button onclick="samSort('status')" id="sam-sort-status"
            style="padding:6px 12px;border:2px solid #9a3412;border-radius:5px;background:#fff;color:#9a3412;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;">
            Status &#8593;
          </button>
          ...
          <button onclick="samSort('count')" id="sam-sort-count"
            style="padding:6px 12px;border:2px solid #9a3412;border-radius:5px;background:#9a3412;color:#fff;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;">
            Eins&#228;tze &#8595;
          </button>
        </div>
      </div>
  ...
  <div id="panel-fa" style="display:none;flex:1;overflow:hidden;background:#fffaf0;font-family:'Segoe UI',Arial,sans-serif;flex-direction:column;">
    ...
    <div style="display:flex;flex:1;overflow:hidden;">
      ...
      <div id="fa-detail-panel" style="flex:1;overflow-y:auto;padding:20px 24px;background:#fffaf0;"></div>
    </div>
  </div>
  ...
  <div id="panel-zulage" style="display:none;flex:1;flex-direction:column;background:#e8ecf1;font-family:'Segoe UI',Arial,sans-serif;overflow:hidden;">
    <div style="display:flex;align-items:center;gap:10px;padding:12px 20px;background:#f0f2f6;border-bottom:1.5px solid #c8cfd9;flex-wrap:wrap;flex-shrink:0;">
      <h2 style="margin:0;font-size:16px;font-weight:900;color:#9a3412;">&#128176; Zulagen</h2>
      <div style="display:flex;gap:4px;">
        <button id="ztab-sonder" onclick="zulagenTab('sonder')" style="padding:3px 9px;border-radius:3px;border:1.5px solid #9a3412;cursor:pointer;font-weight:700;font-size:12px;background:#9a3412;color:#fff;">Sonderfahrzeuge</button>
        <button id="ztab-fuengers" onclick="zulagenTab('fuengers')" style="padding:3px 9px;border-radius:3px;border:1.5px solid #9a3412;cursor:pointer;font-weight:700;font-size:12px;background:#fff;color:#9a3412;">F&#252;ngers</button>
        <button id="ztab-drittkunden" onclick="zulagenTab('drittkunden')" style="padding:3px 9px;border-radius:3px;border:1.5px solid #9a3412;cursor:pointer;font-weight:700;font-size:12px;background:#fff;color:#9a3412;">Drittkunden</button>
      </div>
      <select id="zulage-month-sel" onchange="zulagenRender()" style="padding:5px 12px;border:2px solid #9a3412;border-radius:5px;font-size:12px;outline:none;cursor:pointer;"></select>
  ...
  <!-- ── Spesen Panel ───────────────────────────────────── -->
  <div id="panel-spesen" style="display:none;flex:1;overflow:hidden;background:#fffaf0;font-family:'Segoe UI',Arial,sans-serif;flex-direction:column;">
    <div style="display:flex;align-items:center;gap:10px;padding:10px 16px;background:#f1f5f9;border-bottom:1.5px solid #cbd5e1;flex-wrap:wrap;flex-shrink:0;">
    ...
    <div style="display:flex;flex:1;overflow:hidden;">
      <div style="width:240px;flex-shrink:0;border-right:1.5px solid #cbd5e1;overflow-y:auto;background:#fffaf0;">
        <div id="spesen-sidebar-list"></div>
      </div>
      <div id="spesen-detail-panel" style="flex:1;overflow-y:auto;padding:20px 24px;background:#fffaf0;">
  ...
  <div id="panel-verstoss" style="display:none;flex:1;flex-direction:column;background:#f1f5f9;font-family:'Segoe UI',Arial,sans-serif;overflow:hidden;">
    <div style="width:92%;margin:0 auto;display:flex;flex-direction:column;flex:1;overflow:hidden;">
      <div id="verstoss-toolbar" style="display:flex;align-items:center;gap:10px;padding:14px 0;flex-wrap:wrap;flex-shrink:0;">
        ...
        <input id="verstoss-search" placeholder="Fahrer suchen..." oninput="verstossFilter(this.value)"
          ...
          onfocus="this.style.borderColor='#dc2626'" onblur="this.style.borderColor='#fdba74'">
        ...
        <button id="verstoss-search-all" onclick="verstossResetSearch()"
          style="padding:8px 12px;border:1.5px solid #431407;border-radius:8px;background:#431407;color:#fff;font-size:12px;font-weight:900;cursor:pointer;font-family:inherit;white-space:nowrap;box-shadow:0 1px 2px rgba(15,23,42,.08);">
  ...
  <!-- ── Großkunden Panel ───────────────────────────────────────────────────── -->
  <div id="panel-gk" style="display:none;flex:1;overflow:hidden;background:linear-gradient(135deg,#fff7ed 0%,#fff7ed 52%,#f0fdf4 100%);font-family:'Segoe UI',Arial,sans-serif;flex-direction:column;">
    <!-- Kopf / Suche -->
    <div style="flex-shrink:0;background:linear-gradient(135deg,#ffffff 0%,#ffedd5 58%,#dcfce7 100%);border-bottom:1.5px solid #fdba74;padding:10px 16px;box-shadow:0 2px 10px rgba(14,165,233,.16);">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
        <div style="font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:.45px;color:#9a3412;white-space:nowrap;">Gro&#223;kunden <span id="gk-count" style="font-weight:700;color:#854d0e;"></span></div>
        <input id="gk-search" placeholder="Gro&#223;kunden suchen..." oninput="gkFilter(this.value)"
          style="flex:1;max-width:340px;padding:7px 11px;border:1.5px solid #fdba74;border-radius:6px;font-size:12px;font-family:inherit;outline:none;background:#fff;color:#1f3347;box-shadow:0 1px 3px rgba(14,165,233,.12);">
    ...
    <!-- Detail unten -->
    <div id="gk-detail" style="flex:1;overflow-y:auto;padding:16px 18px 28px;background:linear-gradient(135deg,#fff7ed 0%,#f0fdfa 100%);">
...
function gkDistributorLinkHtml(emails, label, subject) {{
  ...
  return "<a href='" + gkAttr(gkMailto(emails, subject || label)) + "'"
       + " style='display:inline-flex;align-items:center;gap:7px;background:#fff7ed;color:#9a3412;"
       + "border:1px solid #fdba74;border-radius:5px;padding:5px 11px;font-size:11.5px;"
...
function gkBuildTiles(activeIdx) {{
  ...
  visible.forEach(function(realIdx) {{
    ...
    var bg      = active ? "linear-gradient(135deg,#ffedd5 0%,#ffedd5 55%,#dcfce7 100%)" : "#fff";
    var border  = active ? "#fb923c" : "#cbd5e1";
    var txtMain = active ? "#431407" : "#334155";
    var txtSub  = active ? "rgba(67,20,7,.72)" : "#334155";
    var knrBg    = active ? "rgba(154,52,18,.12)" : "#ffedd5";
    var knrTxt   = active ? "#431407" : "#334155";
    var knrLabel = active ? "rgba(124,45,18,.72)" : "#9a3412";
...
function gkRenderStructured(customer, detail) {{
  ...
  html += "<div style='margin-bottom:14px;display:flex;align-items:baseline;gap:14px;flex-wrap:wrap;'>"
        + "<h1 style='font-size:22px;font-weight:800;color:#431407;margin:0;letter-spacing:-.4px;line-height:1.1;'>"
  ...
  function gkKnrBadge(knr) {{
    if (!knr) return "";
    return "<span style='display:inline-flex;align-items:baseline;gap:5px;background:#ffedd5;"
         + "border-radius:4px;padding:3px 9px;'>"
         + "<span style='font-size:8.5px;font-weight:700;text-transform:uppercase;letter-spacing:.55px;color:#9a3412;'>KNr</span>"
         + "<span style='font-size:13px;font-weight:800;color:#431407;font-variant-numeric:tabular-nums;letter-spacing:.2px;line-height:1;'>"
         + gkEsc(knr) + "</span></span>";
  }}

  html += "<div style='background:#fff;border:1px solid #fed7aa;border-radius:8px;margin-bottom:12px;overflow:hidden;box-shadow:0 1px 4px rgba(194,65,12,.08);'>";
  html += "<div style='padding:9px 14px;background:linear-gradient(135deg,#ffedd5 0%,#fef3c7 100%);border-bottom:1px solid #fdba74;'>"
        + "<span style='font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:.7px;color:#9a3412;'>Standorte</span>"
        + "</div>";
  ...
  if (anyHasAddr) {{
    ...
    standorte.forEach(function(s) {{
      html += "<div style='display:inline-flex;align-items:center;gap:9px;padding:7px 11px;background:#fffaf0;border:1px solid #fed7aa;border-radius:6px;'>"
  ...
  if (hasAnyContact && allContactRows.length) {{
    html += "<div style='background:#fff;border:1px solid #fed7aa;border-radius:8px;overflow:hidden;box-shadow:0 1px 4px rgba(194,65,12,.08);'>";
    html += "<div style='padding:9px 14px;background:linear-gradient(135deg,#ffedd5 0%,#fef3c7 100%);border-bottom:1px solid #fdba74;display:flex;align-items:center;gap:10px;flex-wrap:wrap;'>"
          + "<span style='font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:.7px;color:#9a3412;flex:1;'>Kontakte</span>"
    ...
    allContactRows.forEach(function(cr, ri) {{
      if (cr.isLabel) {{
        ...
        html += "<div style='margin:14px 0 6px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding-bottom:5px;border-bottom:1px solid #fed7aa;'>"
              + "<span style='font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:#9a3412;'>"
        ...
        if (hasEmail) {{
          html += cr.emails.map(function(em) {{
            ...
            return isMailAddr
              ? "<a href='mailto:" + gkEsc(em) + "' style='color:#9a3412;font-size:12.5px;font-weight:600;text-decoration:none;'>" + gkEsc(em) + "</a>"
              : "<span style='font-size:12.5px;color:#334155;'>" + gkEsc(em) + "</span>";
          }}).join(" ");
        }} else if (hasLabelText) {{
          html += "<span style='font-size:12.5px;font-weight:600;color:#9a3412;'>" + gkEsc(cr.labelText) + "</span>";
        }} else {{
        ...
        if (hasTel) {{
          html += "<span style='display:flex;align-items:center;gap:6px;flex-shrink:0;padding-left:14px;'>"
                + cr.tels.map(function(t) {{
                    ...
                    return "<a href='" + href + "' style='font-size:12.5px;font-weight:600;color:#9a3412;"
                         + "font-variant-numeric:tabular-nums;white-space:nowrap;text-decoration:none;letter-spacing:.2px;'>"
                         + "<span style='color:#c2410c;font-weight:400;margin-right:4px;'>&#9742;</span>" + gkEsc(t) + "</a>";
                  }}).join(" ")
                + "</span>";
        }}
        html += "</div>";
  ...
  if (allHints.length) {{
    html += "<div style='background:#fff;border:1px solid #fed7aa;border-radius:8px;overflow:hidden;box-shadow:0 1px 4px rgba(194,65,12,.08);'>";
    html += "<div style='padding:9px 14px;background:linear-gradient(135deg,#ffedd5 0%,#fef3c7 100%);border-bottom:1px solid #fdba74;display:flex;align-items:center;gap:8px;'>"
          + "<span style='display:inline-block;width:6px;height:6px;border-radius:50%;background:#9a3412;'></span>"
          + "<span style='font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:.7px;color:#9a3412;flex:1;'>Hinweise</span>"
...
function gkRenderFreeform(customer, detail) {{
  ...
  html += "<div style='margin-bottom:14px;display:flex;align-items:center;gap:14px;flex-wrap:wrap;'>"
        + "<h1 style='font-size:22px;font-weight:800;color:#431407;margin:0;letter-spacing:-.4px;line-height:1.1;'>" + gkEsc(customer.name) + "</h1>"
        + gkEdekaLagerNmsLink(customer)
        + "</div>";
  ...
  (customer.lines || []).forEach(function(line) {{
    ...
    if (isSection) {{
      ...
      html += "<div style='padding:10px 14px 8px;background:linear-gradient(135deg,#ffedd5 0%,#fef3c7 100%);border-bottom:1px solid #fdba74;"
      ...
      html += "<div style='padding:8px 14px;border-bottom:1px solid #f1f5f9;'>"
            + "<a href='mailto:" + gkEsc(t) + "' style='color:#9a3412;font-size:12.5px;font-weight:600;text-decoration:none;'>" + gkEsc(t) + "</a>"
            + "</div>";
    }} else if (hasTelNum) {{
      html += "<div style='padding:8px 14px;border-bottom:1px solid #f1f5f9;font-size:12.5px;font-weight:600;color:#334155;font-variant-numeric:tabular-nums;letter-spacing:.2px;'>"
            + "<span style='color:#c2410c;font-weight:400;margin-right:5px;'>&#9742;</span>" + gkEsc(t) + "</div>";
    }} else if (isLong) {{
...
function telPDF() {{
  ...
  var css = [
    ...
    "h1{{font-size:13pt;color:#9a3412;margin:0 0 4mm 0;border-bottom:2px solid #9a3412;padding-bottom:2mm}}",
    ".gruppe{{font-size:8pt;font-weight:900;text-transform:uppercase;color:#9a3412;margin:4mm 0 1.5mm 0;letter-spacing:.3px;border-bottom:1px solid #9a3412;padding-bottom:0.5mm}}",
    ...
    ".itel{{color:#9a3412;font-size:7.5pt}}",
...
function telHover(el, on) {{
  el.style.background = on ? "#fff7ed" : "#fff";
}}
...
function telRender(q) {{
  ...
  TEL_DATA.forEach(function(g) {{
    ...
    html += "<div style='font-size:11px;font-weight:900;text-transform:uppercase;"
          + "letter-spacing:.5px;color:#9a3412;padding:4px 0;"
          + "border-bottom:2px solid #9a3412;margin-bottom:6px'>";
    ...
    hits.forEach(function(p) {{
      ...
      html += "<a href='tel:" + telClean + "' onclick='event.stopPropagation()'" + " title='ProCall: " + safetel + "'"
            + " style='display:block;font-size:13px;color:#9a3412;font-weight:700;margin-top:2px;text-decoration:none;'>&#128222; " + p.tel + "</a>";
      if(p.mail) {{
        var isRole = (p.mail==="Disponent"||p.mail==="Chef");
        html += isRole
...
function samSort(mode) {{
  ...
  ["name","count","status"].forEach(function(m) {{
    ...
    if(!btn) return;
    btn.style.background = mode===m ? "#9a3412" : "#fff";
    btn.style.color      = mode===m ? "#fff"    : "#9a3412";
  }});
...
function samRender(q) {{
  ...
  var statsHtml =
    ...
    samStatBtn("ok", "&#8679; Im Soll: "+nOk, "#ffedd5", "#9a3412", "Nur Fahrer im Soll anzeigen") +
    ...
    "<span style='margin-left:auto;color:#94a3b8;font-size:10px;'>Soll heute: <b style='color:#9a3412;'>"+soll+"</b> / "+ZIEL+" &nbsp;("+satElapsed+" von "+satTotal+" Samstagen)</span>" +
    "</div>";
  ...
  var statusCfg = {{
    ...
    ok:   {{ border:"#9a3412", bg:"#fff7ed", badge:"#9a3412", soft:"#ffedd5", icon:"↑", label:"Im Soll" }},
  ...
  filtered.forEach(function(d, idx) {{
    ...
    var datesHtml = sortedDaten.length ? sortedDaten.map(function(e) {{
      ...
      return "<span style='display:inline-flex;align-items:center;gap:5px;background:#fff;border:1px solid #d8dee8;border-radius:5px;padding:4px 8px;margin:3px;font-size:10.5px;color:#334155;'>" +
        ...
        " <b style='color:#9a3412;'>" + samEsc(tourTxt) + "</b></span>";
...
(function() {{
  ...
  function _renderVacationBox(vacationRows) {{
    return _renderAbsenceBox(vacationRows, {{ title:"Urlaubstage", border:"#bae6fd", bg:"#f1f5f9", text:"#7c2d12", badgeBg:"#f1f5f9", badgeText:"#7c2d12" }});
  }}
  ...
  function _renderLkwBox(lkwEntries) {{
    ...
    html += "<summary style='cursor:pointer;padding:7px 10px;display:flex;align-items:center;gap:8px;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.45px;color:#431407;'>"
          + "<span>LKW in Auswahl</span>"
          + "<span style='display:inline-flex;align-items:center;justify-content:center;min-width:22px;height:18px;border-radius:4px;background:#f1f5f9;border:1px solid #dbe4ef;color:#431407;font-size:10px;font-weight:900;'>" + lkwEntries.length + "</span>"
    ...
    lkwEntries.forEach(function(e) {{
      html += "<span style='min-height:22px;display:inline-flex;align-items:center;justify-content:space-between;gap:6px;background:#fffaf0;border:1px solid #dbe4ef;border-radius:4px;padding:2px 7px;font-size:10.5px;font-weight:800;color:#431407;font-variant-numeric:tabular-nums;white-space:nowrap;overflow:hidden;'>"
  ...
  function _printShiftStyles() {{
    return "<style>"
      ...
      + ".month-head{{display:flex;align-items:center;gap:8px;flex-wrap:wrap;background:#fffaf0;border-bottom:1px solid #cbd5e1;padding:6px 8px;}}"
      + ".month-title{{font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:.45px;color:#431407;min-width:110px;}}"
  ...
  function _renderShiftRows(rows) {{
    ...
    rows.forEach(function(s, i) {{
      ...
      html += "<td style='padding:5px 7px;text-align:right;font-weight:800;color:#431407;font-variant-numeric:tabular-nums;white-space:nowrap;'>" + faEsc(s.schichtdauer || "") + "</td>";
  ...
  function faRenderShifts(name, panel) {{
    ...
    html += "<div><div style='font-size:9.5px;color:#64748b;text-transform:uppercase;letter-spacing:.45px;font-weight:700;'>Urlaubstage</div>"
          + "<div style='font-size:17px;font-weight:900;color:" + (vacationCount ? "#7c2d12" : "#166534") + ";font-variant-numeric:tabular-nums;line-height:1.15;'>" + vacationCount + "</div></div>";
    html += "<div><div style='font-size:9.5px;color:#64748b;text-transform:uppercase;letter-spacing:.45px;font-weight:700;'>Σ Netto Schichten</div>"
          + "<div style='font-size:17px;font-weight:900;color:#431407;font-variant-numeric:tabular-nums;line-height:1.15;'>" + _fmtMin(stats.netto) + "</div></div>";
    html += "<div><div style='font-size:9.5px;color:#64748b;text-transform:uppercase;letter-spacing:.45px;font-weight:700;'>Urlaub +8h</div>"
          + "<div style='font-size:17px;font-weight:900;color:" + (vacationCredit ? "#7c2d12" : "#166534") + ";font-variant-numeric:tabular-nums;line-height:1.15;'>" + _fmtMin(vacationCredit) + "</div></div>";
    ...
    if (monthFilter !== "all") {{
      ...
      html += "<div style='padding:9px 14px;background:#fffaf0;border-bottom:1px solid #e2e8f0;display:flex;align-items:center;gap:14px;flex-wrap:wrap;'>";
      html += "<span style='font-size:12px;font-weight:800;color:#431407;text-transform:uppercase;letter-spacing:.6px;'>" + faEsc(selectedLabel) + "</span>";
      ...
      monthKeys.forEach(function(mk) {{
        ...
        html += "<div style='background:#fff;border:1px solid #e2e8f0;border-radius:6px;overflow:hidden;margin-bottom:12px;'>";
        html += "<div style='padding:9px 14px;background:#fffaf0;border-bottom:1px solid #e2e8f0;display:flex;align-items:center;gap:14px;flex-wrap:wrap;'>";
        html += "<span style='font-size:12px;font-weight:800;color:#431407;text-transform:uppercase;letter-spacing:.6px;'>" + faEsc(grp.label) + "</span>";
        ...
        html += "<span style='margin-left:auto;font-size:11px;color:#64748b;font-weight:600;'>Σ Netto <b style='color:#431407;font-variant-numeric:tabular-nums;'>" + _fmtMin(gs.netto) + "</b> &nbsp;·&nbsp; Urlaub +8h <b style='color:#7c2d12;font-variant-numeric:tabular-nums;'>" + _fmtMin(gsVacationCredit) + "</b> &nbsp;·&nbsp; Σ inkl. Urlaub <b style='color:#0f172a;font-variant-numeric:tabular-nums;'>" + _fmtMin(gsNettoWithVacation) + "</b> &nbsp;·&nbsp; &gt; 10:00 Netto <b style='color:#be123c;font-variant-numeric:tabular-nums;'>" + gs.over10 + "</b></span>";
  ...
  window.faPrintShiftMonth = function() {{
    ...
    function _printMonthSection(mk, monthLabel, rowList) {{
      ...
      sec += _printAbsenceLine("Urlaubstage (+8:00 je Tag / Σ " + _fmtMin(mVacationCredit) + ")", mVacation, "border-color:#bae6fd;background:#f0f9ff;color:#7c2d12;", function() {{ return " (+8:00)"; }});
  ...
  function _faRender10hFrequency(rows) {{
    ...
    function renderBox(title, sub, headers, rowsHtml, countLabel) {{
      return "<details style='background:#fff;border:1px solid #e2e8f0;border-radius:6px;overflow:hidden;min-width:0;'>"
        + "<summary style='list-style:none;cursor:pointer;padding:10px 12px;background:#fffaf0;border-bottom:1px solid #e2e8f0;display:flex;align-items:center;justify-content:space-between;gap:10px;'>"
  ...
  function _faRender10hRows(rows) {{
    ...
    rows.forEach(function(r, i) {{
      ...
      html += "<td style='padding:7px 9px;text-align:right;color:#431407;font-weight:800;font-variant-numeric:tabular-nums;white-space:nowrap;'>" + faEsc(r.schichtdauer) + "</td>";
  ...
  window.faShow10hTours = function(driverFilter) {{
    ...
    html += "<span style='background:#fffaf0;color:#0f172a;border:1px solid #e2e8f0;border-radius:5px;padding:5px 12px;font-size:12px;font-weight:800;'>Auswahl: " + faEsc(yearLabel) + "</span>";
    html += "<span style='background:#fffaf0;color:#0f172a;border:1px solid #e2e8f0;border-radius:5px;padding:5px 12px;font-size:12px;font-weight:800;'>Σ Netto " + _fmtMin(sumNetto) + "</span>";
    html += "</div></div></div>";
  ...
  window.faBuildSidebarHighlight = function(activeName) {{
    ...
    if (statsEl) {{
      ...
      statsEl.innerHTML = "<b>" + filtered.length + "</b> Fahrer &nbsp;&middot;&nbsp; "
        ...
        + " &nbsp;&middot;&nbsp; Urlaub <b style='color:#7c2d12;'>" + totalVacation + "</b>"
        + " &nbsp;&middot;&nbsp; Σ Netto Schichten <b>" + _fmtMin(totalNetto) + "</b>"
        + " &nbsp;&middot;&nbsp; Urlaub +8h <b style='color:#7c2d12;'>" + _fmtMin(totalVacationCredit) + "</b>"
    ...
    filtered.forEach(function(d) {{
      ...
      var badgeBg = active ? "rgba(17,24,39,.08)" : "#f1f5f9";
      var badgeFg = active ? "#111827" : "#334155";
      var overBg = active ? "rgba(17,24,39,.08)" : "#fee2e2";
      var overFg = active ? "#111827" : "#be123c";
      html += "<div data-fa-driver='" + faEsc(d.name) + "'"
        ...
        + (sick ? "<span style='font-size:8px;font-weight:800;padding:1px 4px;border-radius:2px;background:" + (active ? "rgba(17,24,39,.08)" : "#fee2e2") + ";color:" + (active ? "#111827" : "#be123c") + ";'>K " + sick + "</span>" : "")
        + (vacation ? "<span style='font-size:8px;font-weight:800;padding:1px 4px;border-radius:2px;background:" + (active ? "rgba(17,24,39,.08)" : "#f1f5f9") + ";color:" + (active ? "#111827" : "#7c2d12") + ";'>U " + vacation + "</span>" : "")
