...
APP_CACHE_VERSION = "verstossliste-jahresfilter-2026-05-05-v10-gelb-style"
...
function spesenSort(mode) {
  ...
  ["gesamt","name","tage"].forEach(function(m){
    ...
    if(btn){ btn.style.background = mode === m ? "#5a4200" : "#fff"; btn.style.color = mode === m ? "#111827" : "#5a4200"; }
  });
  spesenFilteredCache = null;
  spesenRefresh();
}
...
function spesenUpdateSidebarHighlight() {
  ...
  for(var i=0; i<items.length; i++){
    ...
    el.style.background = act ? "#5a4200" : "#fff";
...
function spesenShowDetail(name) {
  ...
  html += "<button onclick='spesenPdfOne(" + JSON.stringify(d.name) + ")' "
        + "style='padding:7px 14px;background:linear-gradient(180deg,#fff27a 0%,#fce531 100%);color:#111827;border:none;border-radius:6px;"
  ...
  html += "<thead><tr style='background:#fff9bf;color:#5a4200;text-align:left;'>" +
          "<th style='padding:9px 10px;border-bottom:1px solid #cbd5e1;position:sticky;top:0;background:#fff9bf;z-index:1;'>Datum</th>" +
          "<th style='padding:9px 10px;border-bottom:1px solid #cbd5e1;position:sticky;top:0;background:#fff9bf;z-index:1;'>Start</th>" +
          "<th style='padding:9px 10px;border-bottom:1px solid #cbd5e1;position:sticky;top:0;background:#fff9bf;z-index:1;'>Ende</th>" +
          "<th style='padding:9px 10px;border-bottom:1px solid #cbd5e1;position:sticky;top:0;background:#fff9bf;z-index:1;'>Dauer</th>" +
          "<th style='padding:9px 10px;border-bottom:1px solid #cbd5e1;position:sticky;top:0;background:#fff9bf;z-index:1;'>Fahrzeug</th>" +
          "<th style='padding:9px 10px;border-bottom:1px solid #cbd5e1;position:sticky;top:0;background:#fff9bf;z-index:1;'>Strecke</th>" +
          "<th style='padding:9px 10px;border-bottom:1px solid #cbd5e1;text-align:right;position:sticky;top:0;background:#fff9bf;z-index:1;'>Steuerfrei</th>" +
          "<th style='padding:9px 10px;border-bottom:1px solid #cbd5e1;text-align:right;position:sticky;top:0;background:#fff9bf;z-index:1;'>Steuerpflichtig</th>" +
          "<th style='padding:9px 10px;border-bottom:1px solid #cbd5e1;text-align:right;position:sticky;top:0;background:#fff9bf;z-index:1;'>Gesamt</th>" +
          "</tr></thead><tbody>";
  if(!s.rows.length) {
    ...
    s.rows.forEach(function(r, idx){
      ...
      if(showMonthSeparator && r.month && r.month !== lastMonth){
        html += "<tr style='background:#f1f5f9;'><td colspan='9' style='padding:6px 12px;font-size:11px;font-weight:900;color:#5a4200;text-transform:uppercase;letter-spacing:.4px;border-bottom:1px solid #cbd5e1;'>" + spEsc(spMonthLabel(r.month)) + "</td></tr>";
        lastMonth = r.month;
      }
      ...
      html += "<td style='padding:8px 10px;border-bottom:1px solid #e2e8f0;text-align:right;font-weight:900;color:#5a4200;'>" + spMoney(spTaxFree(r) + spTaxable(r)) + "</td>";
      html += "</tr>";
    });
  }
...
function spesenPdfOne(name) {
  ...
  var css = "@page{size:A4 portrait;margin:11mm}"
    ...
    + ".head{border-bottom:3px solid #5a4200;padding-bottom:4mm;margin-bottom:5mm}"
    + ".head h1{font-size:18pt;color:#5a4200;font-weight:900;margin-bottom:1mm}"
    ...
    + "tbody tr.sep td{background:#fff9bf;color:#5a4200;font-weight:900;font-size:7.5pt;text-transform:uppercase;letter-spacing:.4px;padding:1.4mm 2.5mm;border-bottom:1px solid #cbd5e1}"
  ...
  body += "<div class='kpi'>"
    + "<div><div class='num' style='color:#5a4200;'>" + spMoney(s.total) + "</div><div class='lbl'>Gesamt</div></div>"
  ...
  s.rows.forEach(function(r){
    ...
    body += "<tr" + rowCls + ">"
      ...
      + "<td style='text-align:right;font-weight:800;color:#5a4200;font-variant-numeric:tabular-nums;white-space:nowrap;'>"
...
function spesenShowLateStarts() {
  ...
  html += "<div style='font-size:12px;color:#64748b;margin-top:2px;'>" + spEsc(subtitle) + " · Startzeitfenster <b style='color:#5a4200;'>" + SPESEN_LATE_FROM + " – " + SPESEN_LATE_TO + " Uhr</b></div>";
  html += "</div>";
  html += "<div style='font-size:11px;color:#94a3b8;'>Klick auf Fahrer in Liste &rarr; zurück zur Detailansicht</div>";
  html += "</div>";

  if(!monthKeys.length){
    html += "<div style='margin-top:12px;padding:32px;text-align:center;color:#94a3b8;font-size:13px;background:#f8fafc;border:1px dashed #cbd5e1;border-radius:8px;'>Keine Spätstarts im gewählten Zeitraum.</div>";
    html += "</div>";
    panel.innerHTML = html;
    panel.scrollTop = 0;
    return;
  ...
  var cards = [
    ["20 Uhr Einträge", String(allEntries), "#5a4200"],
    ["Betroffene Fahrer", String(driverCount), "#5a4200"],
    ["Σ Steuerfrei", spMoney(allMeal), "#15803d"],
    ["Σ Steuerpflichtig", spMoney(allNight), "#92400e"],
    ["Σ Gesamt", spMoney(allTotal), "#5a4200"]
  ];
  html += "<div style='display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;margin-top:14px;'>";
  cards.forEach(function(c){
    html += "<div style='background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:10px 12px;'>"
  ...
  monthKeys.forEach(function(k){
    ...
    html += "<div style='padding:10px 14px;border-bottom:1px solid #fff9bf;background:#fffde7;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;'>";
    html += "<div style='font-size:13px;font-weight:900;color:#5a4200;text-transform:uppercase;letter-spacing:.4px;border-left:3px solid #5a4200;padding-left:8px;'>" + spEsc(mo.label || k) + "</div>";
    html += "<div style='font-size:11px;color:#64748b;font-weight:700;'>" + mo.entries.length + " Einträge · " + monthDriverCount + " Fahrer · Σ <b style='color:#5a4200;'>" + spMoney(mo.total) + "</b></div>";
    html += "</div>";
    ...
    mo.entries.forEach(function(e, idx){
      ...
      html += "<tr style='background:" + bg + ";cursor:pointer;' onclick='spesenShowDetail(" + JSON.stringify(e.driver.name) + ")' title='Zur Detailansicht von " + spEsc(e.driver.name) + "'>"
           ...
           + "</td>"
           + "<td style='padding:8px 10px;border-bottom:1px solid #e2e8f0;font-weight:800;color:#5a4200;font-variant-numeric:tabular-nums;white-space:nowrap;'>" + spEsc(r.start_time || "—") + "</td>"
           ...
           + "<td style='padding:8px 10px;border-bottom:1px solid #e2e8f0;text-align:right;font-weight:900;color:#5a4200;font-variant-numeric:tabular-nums;white-space:nowrap;'>" + spMoney(spTaxFree(r) + spTaxable(r)) + "</td>"
           + "</tr>";
    });

    // Summen-Zeile pro Monat
    html += "<tr style='background:#fff9bf;'>"
         + "<td colspan='5' style='padding:9px 10px;text-align:right;font-weight:900;color:#5a4200;text-transform:uppercase;font-size:11px;letter-spacing:.4px;'>Summe " + spEsc(mo.label || k) + "</td>"
         ...
         + "<td style='padding:9px 10px;text-align:right;font-weight:900;color:#5a4200;font-variant-numeric:tabular-nums;'>" + spMoney(mo.total) + "</td>"
         + "</tr>";
    html += "</tbody></table></div>";
  });
...
function verstossRenderGraph() {
  ...
  verstossChart("verstoss-chart-type", {
    ...
    data: {
      ...
      datasets: [{ data: topTypes.map(function(x){ return x[1]; }), backgroundColor: ["#dc2626", "#f97316", "#f59e0b", "#1e3a5f", "#2563eb", "#0891b2", "#16a34a", "#fce531", "#be123c", "#475569", "#0f766e", "#92400e"] }]
    },
...
function verstossRender() {
  ...
  if (stats) {
    ...
    stats.innerHTML =
      ...
      + (sinceLabel ? " <span style='display:inline-flex;align-items:center;gap:6px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:999px;padding:5px 10px;color:#475569;'>seit <b>" + verstossEsc(sinceLabel) + "</b></span>" : "");
  }
  ...
  html += verstossMiniCard("Aktuellster Tag", latestDisplay, topDriver ? ("Top: " + topDriver.name + " · " + topDriver.count) : "Keine Daten", "#1e3a5f", "#f8fafc", "&#128197;");
  html += "</div>";
  ...
  html += "<th style='padding:9px 6px;width:34px;background:#f8fafc;border-bottom:1px solid #e2e8f0;'></th>";
  ...
  drivers.forEach(function(d, i) {
    ...
    var rowBg = isOpen ? "#fffde7" : (isRecent ? "#fff7f7" : (i % 2 === 0 ? "#ffffff" : "#f8fafc"));
    ...
    html += "<tr onclick='verstossToggleDriver(" + JSON.stringify(d.name) + ")' "
          + "style='background:" + rowBg + ";cursor:pointer;transition:background .12s,box-shadow .12s;border-left:5px solid " + (isRecent ? "#dc2626" : (isOpen ? "#1e3a5f" : "transparent")) + ";' "
          + "onmouseover=\"this.style.background='#eef6ff'\" onmouseout=\"this.style.background='" + rowBg + "'\">";
    html += "<td style='padding:11px 4px 11px 10px;text-align:center;color:" + (isOpen ? "#1e3a5f" : "#94a3b8") + ";font-size:12px;border-bottom:1px solid #eef2f7;'>" + (isOpen ? "&#9660;" : "&#9654;") + "</td>";
    ...
    if (isOpen) {
      ...
      html += "<div style='background:#f8fafc;border:1px solid #cbd5e1;border-radius:14px;margin-top:10px;overflow:hidden;box-shadow:0 8px 22px rgba(15,23,42,.10);'>";
      ...
      html += "<div style='display:flex;align-items:center;gap:10px;padding:12px 16px;border-bottom:1px solid #cbd5e1;background:linear-gradient(180deg,#fffde7 0%,#f8fafc 100%);flex-wrap:wrap;'>";
      ...
      if (d.types && d.types.length) {
        ...
        html += "<button onclick='event.stopPropagation();verstossResetTypeFilter()' "
              + "style='background:" + (!_vsTypeFilter ? "#1e3a5f" : "#fff") + ";border:1px solid " + (!_vsTypeFilter ? "#1e3a5f" : "#cbd5e1") + ";color:" + (!_vsTypeFilter ? "#fff" : "#475569") + ";border-radius:999px;padding:4px 10px;font-size:10.5px;font-weight:950;cursor:pointer;font-family:inherit;'>Alle</button>";
      ...
      detailList.forEach(function(v, j) {
        ...
        if (monthLabel !== currentMonth) {
          ...
          html += "<tr style='background:#fff9bf;'><td colspan='8' style='padding:8px 12px;color:#1e3a5f;font-size:10px;font-weight:950;text-transform:uppercase;letter-spacing:.55px;border-top:1px solid #bfdbfe;border-bottom:1px solid #bfdbfe;'>" + verstossEsc(monthLabel) + "</td></tr>";
        }
...
.vz-day-btn{{padding:7px 14px;border:1.5px solid #1d6f42;background:#fff;color:#1d6f42;border-radius:7px;cursor:pointer;font-weight:800;font-size:12px;font-family:'Segoe UI',Arial,sans-serif;transition:all .15s;letter-spacing:.1px}}
.vz-day-btn:hover{{background:#ecf7f1;border-color:#165532}}
.vz-day-btn.active{{background:linear-gradient(180deg,#2a8a55 0%,#1d6f42 100%);color:#fff;border-color:#165532;box-shadow:0 2px 6px rgba(29,111,66,.3)}}
</style>
</head>
<body>
...
<div class="frame-wrap">
      ...
      <div style="background:#fff;border:1px solid #d7dee7;border-radius:12px;padding:18px 22px;box-shadow:0 2px 8px rgba(15,23,42,.06);margin-bottom:18px">
        <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;margin-bottom:16px">
          <div style="display:flex;align-items:center;gap:12px;min-width:0;flex:1">
            <div style="width:40px;height:40px;border-radius:10px;background:linear-gradient(135deg,#1d6f42 0%,#2a8a55 100%);display:flex;align-items:center;justify-content:center;font-size:20px;box-shadow:0 2px 6px rgba(29,111,66,.25);flex-shrink:0">&#128703;</div>
        ...
        <div style="display:flex;gap:18px;align-items:flex-start;flex-wrap:wrap;padding-top:14px;border-top:1px solid #eef2f7">
          <div style="flex:1;min-width:280px">
            <div style="font-size:11px;font-weight:800;color:#1d6f42;margin-bottom:8px;letter-spacing:.4px;text-transform:uppercase">Tag</div>
          ...
          <div style="flex-shrink:0">
            <div style="font-size:11px;font-weight:800;color:#1d6f42;margin-bottom:8px;letter-spacing:.4px;text-transform:uppercase">Datum</div>
            <input id="fw-date-picker" type="date" onchange="fwSetDate(this.value)"
              style="padding:9px 12px;border:1.5px solid #1d6f42;border-radius:7px;font-size:13px;font-weight:700;font-family:inherit;outline:none;background:#fff;color:#0b1220;min-width:180px;box-shadow:inset 0 1px 2px rgba(15,23,42,.04)">
          </div>
        </div>
      </div>
      ...
      <div style="background:#fff;border:1px solid #d7dee7;border-radius:12px;box-shadow:0 2px 8px rgba(15,23,42,.06);overflow:hidden;margin-bottom:14px">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;gap:14px;flex-wrap:wrap;padding:16px 20px;border-bottom:1px solid #eef2f7;background:linear-gradient(180deg,#fafbfc 0%,#ffffff 100%)">
          <div style="display:flex;align-items:center;gap:10px">
            <div style="width:32px;height:32px;border-radius:8px;background:#e8f1fb;color:#1b66b3;display:flex;align-items:center;justify-content:center;font-size:15px">&#128203;</div>
            <div>
        ...
        <div style="padding:14px 20px;border-bottom:1px solid #eef2f7;background:#fbfcfd">
          <select id="fw-overview-driver" onchange="fwRenderOverview()"
            ...
            onfocus="this.style.borderColor='#1d6f42'" onblur="this.style.borderColor='#cbd5e1'"></select>
        </div>
  ...
  <style>/* fahrerauswertung-gelb-schema */
    #panel-fa {{ background:#fffde7 !important; }}
    #panel-fa > div:first-child {{ background:#fff9bf !important; border-bottom-color:#f3c93c !important; }}
    #panel-fa h2 {{ color:#5a4200 !important; }}
    #fa-search, #fa-year-sel {{ border-color:#5a4200 !important; color:#5a4200 !important; }}
    ...
    #panel-fa > div:nth-child(2) > div:first-child {{ background:#fffde7 !important; border-right-color:#f3c93c !important; }}
    #fa-detail-panel {{ background:#fffde7 !important; }}
    #fa-detail-panel thead tr {{ background:linear-gradient(180deg,#fff27a 0%,#fce531 100%) !important; }}
    #fa-detail-panel .fa-tabs button {{ color:#5a4200; }}
    ...
    #panel-fa button[style*="background:#fce531"],
  ...
  <div id="panel-fa" style="display:none;flex:1;overflow:hidden;background:#fffde7;font-family:'Segoe UI',Arial,sans-serif;flex-direction:column;">
    <div style="display:flex;align-items:center;gap:10px;padding:10px 16px;background:#fff9bf;border-bottom:1.5px solid #f3c93c;flex-wrap:wrap;flex-shrink:0;">
      <h2 style="color:#5a4200;font-size:16px;font-weight:900;margin:0;">&#128101; Schichten / Tachograph</h2>
      <input id="fa-search" placeholder="Fahrer suchen..." oninput="faRender(this.value);"
        style="flex:1;min-width:130px;max-width:200px;padding:5px 12px;border:2px solid #fce531;border-radius:5px;font-size:12px;font-family:inherit;outline:none">
      <select id="fa-year-sel" onchange="faYearChange(this.value)"
        style="padding:5px 10px;border:2px solid #fce531;border-radius:5px;font-size:12px;font-weight:700;color:#5a4200;cursor:pointer;font-family:inherit;outline:none;background:#fff;">
        </select>
    ...
    <div style="display:flex;flex:1;overflow:hidden;">
      <div style="width:220px;flex-shrink:0;border-right:1.5px solid #f3c93c;overflow-y:auto;background:#fff9bf;">
        <div id="fa-sidebar-list"></div>
      </div>
      <div id="fa-detail-panel" style="flex:1;overflow-y:auto;padding:20px 24px;background:#fffde7;"></div>
    </div>
  </div>
  ...
  <!-- ── Spesen Panel ───────────────────────────────────── -->
  <div id="panel-spesen" style="display:none;flex:1;overflow:hidden;background:#fffde7;font-family:'Segoe UI',Arial,sans-serif;flex-direction:column;">
    <div style="display:flex;align-items:center;gap:10px;padding:10px 16px;background:#fff9bf;border-bottom:1.5px solid #f3c93c;flex-wrap:wrap;flex-shrink:0;">
      <h2 style="color:#5a4200;font-size:16px;font-weight:900;margin:0;">&#128181; Spesen</h2>
      <input id="spesen-search" placeholder="Fahrer suchen..." oninput="spesenFilter(this.value)"
        style="flex:1;min-width:130px;max-width:220px;padding:5px 12px;border:2px solid #fce531;border-radius:5px;font-size:12px;font-family:inherit;outline:none">
      <select id="spesen-month-sel" onchange="spesenMonthChange(this.value)"
        style="padding:5px 10px;border:2px solid #fce531;border-radius:5px;font-size:12px;font-weight:700;color:#5a4200;cursor:pointer;font-family:inherit;outline:none;background:#fff;">
      </select>
      <button onclick="spesenResetView()"
        style="padding:5px 12px;border:2px solid #fce531;border-radius:5px;background:#fce531;color:#111827;font-size:11px;font-weight:900;cursor:pointer;font-family:inherit;white-space:nowrap;box-shadow:0 1px 3px rgba(252,229,49,.25);">Reset</button>
      <div id="spesen-stats" style="font-size:11px;color:#64748b;margin-left:auto;font-weight:700;"></div>
    </div>
    <div style="display:flex;flex:1;overflow:hidden;">
      <div style="width:240px;flex-shrink:0;border-right:1.5px solid #f3c93c;overflow-y:auto;background:#fffde7;">
        <div id="spesen-sidebar-list"></div>
      </div>
      <div id="spesen-detail-panel" style="flex:1;overflow-y:auto;padding:20px 24px;background:#fffde7;">
  ...
  <!-- ── Großkunden Panel ───────────────────────────────────────────────────── -->
  <div id="panel-gk" style="display:none;flex:1;overflow:hidden;background:#fffde7;font-family:'Segoe UI',Arial,sans-serif;flex-direction:column;">
    <!-- Kopf / Suche -->
    <div style="flex-shrink:0;background:#fff9bf;border-bottom:1.5px solid #f3c93c;padding:10px 16px;box-shadow:0 1px 4px rgba(252,229,49,.08);">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
        <div style="font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:.45px;color:#5a4200;white-space:nowrap;">Gro&#223;kunden <span id="gk-count" style="font-weight:700;color:#5a4200;"></span></div>
        <input id="gk-search" placeholder="Gro&#223;kunden suchen..." oninput="gkFilter(this.value)"
          style="flex:1;max-width:320px;padding:7px 11px;border:1.5px solid #fce531;border-radius:6px;font-size:12px;font-family:inherit;outline:none;background:#fff;color:#5a4200;box-shadow:0 1px 3px rgba(252,229,49,.10);">
    ...
    <!-- Detail unten -->
    <div id="gk-detail" style="flex:1;overflow-y:auto;padding:16px 18px 28px;background:#fffde7;">
...
function gkDistributorLinkHtml(emails, label, subject) {{
  ...
  return "<a href='" + gkAttr(gkMailto(emails, subject || label)) + "'"
       + " style='display:inline-flex;align-items:center;gap:7px;background:#fce531;color:#111827;"
       + "border:1px solid #5a4200;border-radius:5px;padding:5px 11px;font-size:11.5px;"
       + "font-weight:800;text-decoration:none;white-space:nowrap;box-shadow:0 1px 3px rgba(252,229,49,.25);'>"
...
function gkBuildTiles(activeIdx) {{
  ...
  visible.forEach(function(realIdx) {{
    ...
    var bg      = active ? "linear-gradient(180deg,#fff27a 0%,#fce531 100%)" : "#fff";
    var border  = active ? "#fce531" : "#f3c93c";
    var txtMain = active ? "#111827" : "#5a4200";
    var txtSub  = active ? "rgba(17,24,39,.72)" : "#5a4200";
    var knrBg    = active ? "rgba(17,24,39,.08)" : "#fff9bf";
    var knrTxt   = active ? "#111827" : "#5a4200";
    var knrLabel = active ? "rgba(17,24,39,.62)" : "#9a4e00";
    html += "<button type='button' onclick='gkShow(" + realIdx + ")'"
          ...
          + (active ? "box-shadow:0 2px 8px rgba(252,229,49,.28);" : "")
          + "'>"
...
function gkRenderStructured(customer, detail) {{
  ...
  html += "<div style='margin-bottom:14px;display:flex;align-items:baseline;gap:14px;flex-wrap:wrap;'>"
        + "<h1 style='font-size:22px;font-weight:800;color:#5a4200;margin:0;letter-spacing:-.4px;line-height:1.1;'>"
  ...
  function gkKnrBadge(knr) {{
    if (!knr) return "";
    return "<span style='display:inline-flex;align-items:baseline;gap:5px;background:#fff9bf;"
         + "border-radius:4px;padding:3px 9px;'>"
         + "<span style='font-size:8.5px;font-weight:700;text-transform:uppercase;letter-spacing:.55px;color:#94a3b8;'>KNr</span>"
         + "<span style='font-size:13px;font-weight:800;color:#5a4200;font-variant-numeric:tabular-nums;letter-spacing:.2px;line-height:1;'>"
         + gkEsc(knr) + "</span></span>";
  }}

  html += "<div style='background:#fff;border:1px solid #f3c93c;border-radius:8px;margin-bottom:12px;overflow:hidden;'>";
  html += "<div style='padding:9px 14px;background:#fff9bf;border-bottom:1px solid #f3c93c;'>"
  ...
  if (anyHasAddr) {{
    ...
    standorte.forEach(function(s) {{
      html += "<div style='display:inline-flex;align-items:center;gap:9px;padding:7px 11px;background:#fff9bf;border:1px solid #f3c93c;border-radius:6px;'>"
  ...
  if (hasAnyContact && allContactRows.length) {{
    html += "<div style='background:#fff;border:1px solid #f3c93c;border-radius:8px;overflow:hidden;'>";
    html += "<div style='padding:9px 14px;background:#fff9bf;border-bottom:1px solid #f3c93c;display:flex;align-items:center;gap:10px;flex-wrap:wrap;'>"
    ...
    allContactRows.forEach(function(cr, ri) {{
      if (cr.isLabel) {{
        ...
        html += "<div style='margin:14px 0 6px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding-bottom:5px;border-bottom:1px solid #f3c93c;'>"
              + "<span style='font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:#5a4200;'>"
        ...
        if (hasEmail) {{
          html += cr.emails.map(function(em) {{
            ...
            return isMailAddr
              ? "<a href='mailto:" + gkEsc(em) + "' style='color:#5a4200;font-size:12.5px;font-weight:500;text-decoration:none;'>" + gkEsc(em) + "</a>"
              : "<span style='font-size:12.5px;color:#334155;'>" + gkEsc(em) + "</span>";
          }}).join(" ");
        }} else if (hasLabelText) {{
          html += "<span style='font-size:12.5px;font-weight:600;color:#334155;'>" + gkEsc(cr.labelText) + "</span>";
        }} else {{
        ...
        if (hasTel) {{
          html += "<span style='display:flex;align-items:center;gap:6px;flex-shrink:0;padding-left:14px;'>"
                + cr.tels.map(function(t) {{
                    ...
                    return "<a href='" + href + "' style='font-size:12.5px;font-weight:600;color:#5a4200;"
  ...
  if (allHints.length) {{
    html += "<div style='background:#fff;border:1px solid #f3c93c;border-radius:8px;overflow:hidden;'>";
    html += "<div style='padding:9px 14px;background:#fff9bf;border-bottom:1px solid #f3c93c;display:flex;align-items:center;gap:8px;'>"
          + "<span style='display:inline-block;width:6px;height:6px;border-radius:50%;background:#fce531;'></span>"
...
function gkRenderFreeform(customer, detail) {{
  ...
  html += "<div style='margin-bottom:14px;display:flex;align-items:center;gap:14px;flex-wrap:wrap;'>"
        + "<h1 style='font-size:22px;font-weight:800;color:#5a4200;margin:0;letter-spacing:-.4px;line-height:1.1;'>" + gkEsc(customer.name) + "</h1>"
        + gkEdekaLagerNmsLink(customer)
        + "</div>";
  ...
  (customer.lines || []).forEach(function(line) {{
    ...
    if (isSection) {{
      ...
      html += "<div style='padding:10px 14px 8px;background:#fff9bf;border-bottom:1px solid #f3c93c;"
      ...
      html += "<div style='padding:8px 14px;border-bottom:1px solid #f1f5f9;'>"
            + "<a href='mailto:" + gkEsc(t) + "' style='color:#5a4200;font-size:12.5px;font-weight:500;text-decoration:none;'>" + gkEsc(t) + "</a>"
            + "</div>";
    }} else if (hasTelNum) {{
      html += "<div style='padding:8px 14px;border-bottom:1px solid #f1f5f9;font-size:12.5px;font-weight:600;color:#5a4200;font-variant-numeric:tabular-nums;letter-spacing:.2px;'>"
      ...
      html += "<div style='padding:10px 14px;border-bottom:1px solid #f1f5f9;"
            + "background:#fffde7;"
            + "font-size:12.5px;color:#5a4200;line-height:1.6;'>" + gkEsc(t) + "</div>";
    }} else {{
...
(function() {{
  ...
  function faTabBar(driverName, hasShifts, hasUebersicht) {{
    ...
    tabs.forEach(function(t) {{
      ...
      html += "<button type='button' "
            ...
            + "color:" + (active ? "#5a4200" : "#64748b") + ";"
            + "border-bottom:2px solid " + (active ? "#5a4200" : "transparent") + ";"
  ...
  function _renderVacationBox(vacationRows) {{
    return _renderAbsenceBox(vacationRows, {{ title:"Urlaubstage", border:"#bae6fd", bg:"#fff9bf", text:"#075985", badgeBg:"#fff9bf", badgeText:"#075985" }});
  }}
  ...
  window.faSort = function(mode) {{
    ...
    ["name","arbeit"].forEach(function(m) {{
      ...
      if (!btn) return;
      btn.style.background = mode === m ? "#5a4200" : "#fff";
      btn.style.color = mode === m ? "#111827" : "#5a4200";
    }});
    faBuildSidebarHighlight(null);
  }};
  ...
  function _faSet10hButton(active) {{
    ...
    // Wichtig: CSS aus dem gelben Schema arbeitet mit !important.
    // Deshalb auch hier setProperty(..., "important"), sonst wird der Text
    // beim aktiven Button gelb auf gelb und wirkt verschwunden.
    btn.style.setProperty("background", active ? "#dc2626" : "#fff", "important");
    btn.style.setProperty("color", active ? "#111827" : "#dc2626", "important");
  ...
  function _faRender10hRows(rows) {{
    ...
    html += "<thead><tr style='background:#fff1f2;color:#5a4200;'>";
  ...
  window.faShow10hTours = function(driverFilter) {{
    ...
    var yearLabel = faYearFilter === "all" ? "Alle Jahre" : faYearFilter;
    var html = "";
    html += "<div style='background:#fff;border:1.5px solid #f3c93c;border-radius:5px;padding:14px 18px;margin-bottom:12px;'>";
    html += "<div style='display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;'>";
    html += "<div><div style='font-size:20px;font-weight:950;color:#5a4200;'>10H Touren" + (selectedDriver ? " · " + faEsc(selectedDriver) : "") + "</div>";
    ...
    html += "<button type='button' onclick='faExport10hPdf()' style='border:2px solid #fce531;background:#fce531;color:#111827;border-radius:5px;padding:5px 12px;font-size:12px;font-weight:950;cursor:pointer;font-family:inherit;'>PDF Tour-Häufigkeit</button>";
    ...
    html += "<button type='button' onclick='faExport10hDriverPdf()' style='border:2px solid #fce531;background:#fce531;color:#111827;border-radius:5px;padding:5px 12px;font-size:12px;font-weight:950;cursor:pointer;font-family:inherit;'>PDF Fahrer</button>";
    html += "<span style='background:#fff9bf;color:#5a4200;border:1px solid #f3c93c;border-radius:5px;padding:5px 12px;font-size:12px;font-weight:900;'>" + rows.length + " Treffer</span>";
  ...
  window.faBuildSidebarHighlight = function(activeName) {{
    ...
    filtered.forEach(function(d) {{
      ...
      var badgeBg = active ? "rgba(17,24,39,.08)" : "#fff9bf";
      var badgeFg = active ? "#111827" : "#5a4200";
      ...
      html += "<div data-fa-driver='" + faEsc(d.name) + "'"
        ...
        + (vacation ? "<span style='font-size:8px;font-weight:800;padding:1px 4px;border-radius:2px;background:" + (active ? "rgba(17,24,39,.08)" : "#fff9bf") + ";color:" + (active ? "#111827" : "#075985") + ";'>U " + vacation + "</span>" : "")
