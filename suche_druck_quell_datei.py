...
function fwComputeRanking() {
  ...
  fwAllKnownDrivers().forEach(function(name) {
    var key = fwDriverNameKey(name);
    if(key && !byDriver[key]) byDriver[key] = { name: name, washKeys: {}, lkws: {} };
  });
  rows.forEach(function(r, idx) {
    ...
    byDriver[driverKey].washKeys[key] = 1;
    if(fz) byDriver[driverKey].lkws[fz] = 1;
  });
  var arr = [];
...
function fwExportDriverPdf(driverName) {
  ...
  var rows = all.filter(function(r){ return (r.fahrer || "").trim() === driverName; });
  if(!rows.length) {
